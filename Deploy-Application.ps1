...
Try {
	...
	[string]$appVendor = 'BMI Imaging Systems'
	[string]$appName = 'DigitalReel (ASR & RCR)'
	...
	$32bitAppInstallName = "DigitalReel (ASR & RCR)" 
	# Application install Path.
	$32bitAppInstallPath = "${Env:ProgramFiles}\Tuolumne County\DigitalReel (ASR & RCR)"
	# Application install parameters.
	...
	$32bitAppUninstallName = "DigitalReel (ASR & RCR)"
	# Application uninstall path.
	...
	[string[]]$appShortcutDirs = @("${Env:ProgramFiles}\Tuolumne County\DigitalReel (ASR & RCR)", "${Env:ProgramData}\Microsoft\Windows\Start Menu\Programs", "${Env:PUBLIC}\Desktop")
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		Try {
			...
			Write-log -Message " 32bitAppInstallName: $32bitAppInstallName | 32bitAppInstallPath: $32bitAppInstallPath"
		...
		Foreach ($appShortcutDir in $appShortcutDirs) {
			Try {
				...
				New-Shortcut -Path "$appShortcutDir\$32bitAppInstallName.lnk" -TargetPath "CONFIDENTIAL" -IconLocation "$envProgramFiles\Tuolumne County\DigitalReel\Assets\DigitalReel_ico.ico" -WindowStyle "Maximized" -Description "DigitalReel"
			}
